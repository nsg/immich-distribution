...
This page describes the upgrade process as a **developer** that I follow when I release a new version of Immich Distribution. If you like to contribute and help me with an update, follow this guide.

{% admonition(type="danger", title="This page is for developers") %}
This page **does not** describe the update process done as a user. That's automatic! :partying_face: For update and user focused instructions see the [install page](/installation/#updates).
{% end %}
...
Immich does **not** follow strict [semantic versioning](https://semver.org/) but overall the patch releases are usually fixes and **minor** features that do not usually affect Immich Distribution.

## Bump Immich version

A GitHub Action will automatically create a PR with relevant files.
...
If you are not experienced with snap packaging this can be hard to follow. If you are curious and like to learn just ask me! If this is not your cup of tea, there are also a few steps that do not require that much understanding about snaps.
{% end %}

I usually start by reading the [release notes](https://github.com/immich-app/immich/releases) to figure out if I need to do something special. Usually are pure code changes, like changes internally in the server, just fine. Changes to startup scripts, dependencies, paths, environment do possibly require changes to this package.

Depending on the release size I have found it useful to look at code changes as well, at least to see what files the release touches. I have written a small tool to track important files called [update.sh](https://github.com/nsg/immich-distribution/blob/master/update.sh), the output of it is included in the pull request.

This script will diff files that I like to keep track on, for example:

* Dockerfiles - I keep track on them to detect changes to dependencies, changes to startup scripts and various things that _may_ affect the snap. If the base image changes, inspect if it contains any relevant changes.
* Scripts - I do not use these scripts so I need to adapt whatever they do to my startup scripts
...
This requires that you have snapcraft installed and configured. I use LXD to build the package, but multipass should work just fine. This is not essential, if the tests pass in GitHub Actions for a simple version bump I consider that fine. For everything else, please test it locally.
{% end %}
...
This builds and installs the unsigned snap package. This is equivalent to the real releases, but it is slower and less convenient compared to the snap try option.
{% end %}

#### Use "snapcraft"

You can of course also use `snapcraft` directly like any other snap package if you prefer, and then manually install it.
...
The snap tests cover the most common use cases, so I usually only do manual tests to make sure that everything "looks good", and to troubleshoot problems.
...
Note that the package needs to be installed for the `*` to expand properly.
...
I release the revision in the `edge` channel to `beta`. I ask the beta testers to test the beta release and inspect the logs closely and make sure that everything works.
...
If everything looks fine, I will release it to `candidate`. My production deployment uses this channel. I will live with this release to make sure that everything works.

## Release to stable

If everything works for me, with no known problems, I will release this to the `stable` channel. I usually release this 2-3 days after candidate, but it may be faster if the mobile client is broken (that's unusual).

## The tracking issue

Keep it open for a few days for users to see, read and possibly comment.
